...
should clear to 0.
    ...
    unsigned long _avr_timer_M =
        1; // Start count from here, down to 0. Default 1ms
...
void TimerOn() {
  ...
  // 00001011: 0x0B
  // SO, 16 MHz clock or 16,000,000 /64 = 250,000
  ticks /
      s
          ...
          // 00001010: 0x0A
          // SO, 16 MHz clock or 16,000,000 /8 = 2,000,000
          ticks /
      s
          // Thus, TCNT1 register will count at 2,000,000
          ticks /
      s
          // AVR output compare register OCR1A.
          OCR2A = 250; // Timer interrupt will be generated when
  TCNT1 == OCR1A
      // We want a 1 ms tick. 0.001 s * 250,000 ticks/s =
      250
      ...
      // We want a 1 us tick. 0.000001 s * 2,000,000 ticks/
      s = 2
...
ISR(TIMER2_COMPA_vect) {
  // CPU automatically calls when TCNT0 == OCR0 (every 1 ms per TimerOn
settings)
