...
public:
    ...
    void drawChar(int x, int y, char c, int color, int bgColor, uint8_t size) {
        if (c < 32 || c > 127) return; // Ensure valid ASCII range
    ...
    void drawString(int x, int y, const char *str, int color, int bgColor, uint8_t size) {
        while (*str) {
...
private:
    ...
    void setPixel(int x, int y, int color) {
        if (x < 0 || x >= MAX_X || y < 0 || y >= MAX_Y) return; // Boundary check
