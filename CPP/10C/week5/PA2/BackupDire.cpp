  ...
  if (root == nullptr) {
    ...
    while (curr != nullptr) {
      if (insertNode->getData() == curr->getData()) {
        curr->incrementCount(); // Dupe!
        return;
      }

      if (insertNode->getData() < curr->getData()) { // Left of head
        if (!curr->getLeft()) {                      // Prevents segfaults
...
// Inclass again
// Tuesday
// We will assume this is bulletproof
void BSTree::remove(const string &key) { root = fix(root, key); }
...
Node *BSTree::fix(Node *curr, const string &key) {
  ...
  if (key > curr->getData()) {
    ...
    // Additional handling for key == curr->getData()
    if (curr->isLeaf()) {
      delete curr;
      return nullptr;
    } else if (curr->onlyLeft()) {
  ...
  // If key > curr->getData(), the function should return curr
  return curr;
}
